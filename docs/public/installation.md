...
**Note:** The operator is installed to the same namespace as the Kafka cluster. To specify which namespaces the KafkaUser or AKHQ controllers should watch, use the `operator.kafkaUserConfigurator.watchNamespace` and `operator.akhqConfigurator.watchNamespace` parameters respectively. By default, both controllers watch all Kubernetes namespaces.
